...
@never_cache
@login_required
def concession_request(request):
    user1 = request.user
    try:
        user = tbl_stationmaster.objects.get(user=user1)
        place = user.master_place
        dist = tbl_place.objects.get(place_name=place)
        dd = dist.place_name
        data = Concession.objects.filter(start_place=dist)
    except tbl_stationmaster.DoesNotExist:
        return render(request, 'StationMaster/concession_request.html', {'error': 'Station master not found.'})
    except tbl_place.DoesNotExist:
        return render(request, 'StationMaster/concession_request.html', {'error': 'Place not found.'})
    return render(request, 'StationMaster/concession_request.html', {'data': data})

@never_cache
@login_required
def reject_concession(request):
    if request.method == "POST":
        id=request.POST.get("id")
        data = Concession.objects.get(id=id)
        data.status="Rejected"
        data.save()
        return redirect('WebStationMaster:concession_request')
    return redirect('WebStationMaster:concession_request')
@never_cache
@login_required
def approve_concession(request):
    if request.method == "POST":
        id=request.POST.get("id")
        data = Concession.objects.get(id=id)
...
def boat(request):
    ...
    return render(request, 'StationMaster/Boat.html', {'Data': BoatData})
...
def event_type(request):
    ...
    return render(request, 'StationMaster/EventType.html', {'Data': eventobj})