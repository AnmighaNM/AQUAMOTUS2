...
def add_canteen(request):
    active_places = tbl_place.objects.filter(status=1)
    canteendata = tbl_canteen.objects.all()
    if request.method == "POST":
        name = request.POST.get("name")
        email = request.POST.get("email")
        place = request.POST.get("place")
        p_id = tbl_place.objects.get(id=place)
        def generate_pin(length=6):
            return ''.join(str(random.randint(0, 9)) for _ in range(length))
        password = generate_pin()
        subject = 'Welcome to Aqua Motus'
        message = f"Dear {name},\n\nYou have been successfully registered as a  in Aqua Motus.\nYour default password is: {password}\n\nPlease log in and change your password at your earliest convenience.\n\nBest regards,\nAqua Motus Team"
        from_email = settings.EMAIL_HOST_USER
        recipient_list = [email]
        ...
        canteen.save()
        return render(request, 'WebAdmin/add_canteen.html',{"canteen": canteendata})
    return render(request, 'WebAdmin/add_canteen.html',{'active_places': active_places, "canteen": canteendata})

def toggle_canteen_status(request, pid):
    canteen = tbl_canteen.objects.get(id=pid)
    canteen.status = 1 if canteen.status == 0 else 0
    canteen.save()
    return redirect('WebAdmin:add_canteen')