...
def concession(request):
    ...
    if request.method == "POST":
        ...
        s_place=request.POST.get("start_place")
        e_place=request.POST.get("end_place")
        start_place=tbl_place.objects.get(place_name=s_place)
        end_place=tbl_place.objects.get(place_name=e_place)
        status = "pending"
        ...
        try:
            ...
            concession.save()
            return render(request, 'User/Concession.html', {'user': user, 'profile': profile, 'message': 'Concession saved successfully.', 'places': places})
        
        except Exception as e:
            # Log the error for debugging
            print(f"Error saving concession: {e}")
            return render(request, 'User/Concession.html', {'user': user, 'profile': profile, 'message': 'Failed to save the concession.', 'places': places})

    # Fetch concessions related to the current user
    concessions = Concession.objects.filter(email=user)
    
    # Render the template for GET or if POST fails
    return render(request, 'User/Concession.html', {'user': user, 'profile': profile, 'concession_list': concessions, 'places': places})

# def home(request):
#     return render(request, 'User/Homefd.html')
...
#     return render(request, 'Canteen/addtocart.html',{'menus':menu,'user':user,'places':place})
...
#         return render(request, 'Canteen/addtocart.html',{'menus':data, 'user':user})
#     menus=fd_menu.objects.all()
#     return render(request, 'Canteen/addtocart.html',{'menus':menus,'user':user})
...
#         return render(request, 'Canteen/cart.html',{'message':'Item Added to Cart Successfully!','user':user,'cart':cart})
#     return render(request, 'Canteen/addtocart.html',{'user':user})

# def viewcart(request):
#     user=request.user
#     cart = fd_cart.objects.filter(user_id=user)
#     return render(request, 'Canteen/cart.html',{'user':user,'cart':cart})
...
#     return render(request, 'Canteen/buy.html',{'user':user,'cart':cart,'total':total})
...
#         data.delete()
#         return render(request,'Canteen/cart.html',{'message':'Item Deleted Successfully!','user':user,'cart':cart})
